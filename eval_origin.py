...
from dataloader import EvalPre
...
class SegEvaluator(Evaluator):
    def func_per_iteration(self, data, device):
        ...
        pred = self.whole_eval(img, output_size = None, input_size=None, device=None)
        ...
        if self.show_image:
            ...
            #cv2.waitKey(0)
            cv2.imwrite(os.path.join(self.save_path, fn), cv2.cvtColor(comp_img,cv2.COLOR_RGB2BGR))
            logger.info('Save the image ' + fn)

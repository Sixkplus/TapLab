...
def main():
    ...
    # ------------------------Inference--------------------------
    # -----------------------------------------------------------
    frames = glob.glob(cur_dir_name+'/rgb_cont/*.png')
    ...
    for curGopIdx in range(max(NUM_FRAMES // GOP_FRAMES_NUM, 1)):
        ...
        for innerGopIdx in range(GOP_FRAMES_NUM):
            ...
            img_origin = cv2.imread(cur_dir_name + "/rgb_cont" + '/frame'+ str(curFrameIdx) + '.png').astype(np.float32)
            img = img_origin[:,:,::-1]
            ...
            # P-frame
                ...
                # Load motion vector
                save_mvPng = imread(cur_dir_name + "/mv_cont" + '/frame'+ str(curFrameIdx) + '.png' ).astype(np.int16)
                ...
                if args.rgc or args.rgfs:
                    # load residual map
                    res = imread(cur_dir_name + "/res_cont"+ '/frame'+ str(curFrameIdx) + '.png').astype(np.float32)
                    res = abs(res * 2 - 255)

...
def get_train_loader(engine, dataset, config=config):
...
def get_eval_loader(engine, dataset, batch_size=4):
    data_setting = {'img_root': config.img_root_folder,
                    'gt_root': config.gt_root_folder,
                    'train_source': config.train_source,
                    'eval_source': config.eval_source}
    # train_preprocess = TrainPre(config.image_mean, config.image_std)
    eval_preprocess = EvalPre(config.image_mean, config.image_std)

    eval_dataset = dataset(data_setting, "val", eval_preprocess)
